...
const EarlyLeavePage = ({ userData, settings, earlyLeaves, leaves, onSave, onCancel, addToastContainer, addActivity }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const updatedEarlyLeaves = [...earlyLeaves, newEarlyLeave];
    storeSet('earlyLeaves', updatedEarlyLeaves);
    addActivity(`تم تسجيل إذن انصراف: ${formData.title}`);
    addToast('تم تسجيل إذن الانصراف بنجاح', 'success');
    onSave?.();
  };

  return (
    <div style={{ padding: '20px' }}>
      <h2 style={{ marginBottom: '20px', fontSize: '20px', fontWeight: '700' }}>تسجيل إذن انصراف مبكر</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: '16px' }}>
          <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>وقت الانصراف</label>
          <input
            type="time"
            value={formData.time}
